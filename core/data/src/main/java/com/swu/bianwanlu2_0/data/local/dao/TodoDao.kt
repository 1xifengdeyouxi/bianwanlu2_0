...
interface TodoDao {
    ...
    @Query("SELECT * FROM todos WHERE user_id = :userId AND category_id = :categoryId ORDER BY sort_order DESC, updated_at DESC")
    fun getByUserAndCategory(userId: Long, categoryId: Long): Flow<List<Todo>>

    @Query("SELECT * FROM todos WHERE user_id = :userId AND status = 0 ORDER BY sort_order DESC, updated_at DESC")
    fun getActiveByUser(userId: Long): Flow<List<Todo>>

    @Query("SELECT * FROM todos WHERE user_id = :userId AND status = 0 AND reminder_time IS NOT NULL AND reminder_time < :now ORDER BY sort_order DESC, updated_at DESC")
    fun getExpiredByUser(userId: Long, now: Long): Flow<List<Todo>>

    @Query("SELECT * FROM todos WHERE user_id = :userId AND reminder_time BETWEEN :dayStart AND :dayEnd ORDER BY sort_order DESC, reminder_time ASC")
    fun getTodayByUser(userId: Long, dayStart: Long, dayEnd: Long): Flow<List<Todo>>

    @Query("SELECT * FROM todos WHERE user_id = :userId AND reminder_time BETWEEN :start AND :end ORDER BY sort_order DESC, reminder_time ASC")
    fun getByDateRange(userId: Long, start: Long, end: Long): Flow<List<Todo>>

    @Query("SELECT * FROM todos WHERE user_id = :userId AND is_priority = 1 ORDER BY sort_order DESC, updated_at DESC")
