...
fun CategoryManageScreen(
    ...
    deletingCategory?.let { request ->
        ...
        ConfirmDialog(
            title = "删除分类",
            message = "确定删除“${request.category.name}”吗？删除后，该分类下的${itemLabel}会自动移动到“${request.fallbackCategory.name}”下。",
            confirmText = "删除并迁移",
...
private fun CategoryManageSection(
    ...
    val defaultItemStepPx = with(LocalDensity.current) { 72.dp.toPx() }

    LaunchedEffect(categories, isDragging, pendingReorderedIds) {
        if (isDragging) return@LaunchedEffect

        val currentOrderIds = categories.map { it.id }
    ...
    fun itemStep(categoryId: Long): Float = itemHeights[categoryId] ?: defaultItemStepPx
    ...
    fun handleDrag(category: Category, deltaY: Float) {
        ...
        while (dragOffsetY >= itemStep(category.id) / 2f && currentIndex < localCategories.lastIndex) {
            val movedCategory = localCategories[currentIndex + 1]
            val movedOffset = itemStep(movedCategory.id)
            localCategories.move(currentIndex, currentIndex + 1)
            dragOffsetY -= movedOffset
            recordSwap(movedCategory.id, movedOffset)
            currentIndex++
        }

        while (dragOffsetY <= -itemStep(category.id) / 2f && currentIndex > 0) {
            val movedCategory = localCategories[currentIndex - 1]
            val movedOffset = itemStep(movedCategory.id)
            localCategories.move(currentIndex, currentIndex - 1)
            dragOffsetY += movedOffset
        ...
        localCategories.forEach { category ->
            key(category.id) {
                Box(
                    modifier = Modifier
                        .fillMaxWidth()
                        .zIndex(if (draggingCategoryId == category.id) 20f else 0f)
                ) {
                    Column {
                        CategoryManageRow(
                            category = category,
                            ...
                            onHeightChanged = { height -> itemHeights[category.id] = height },
...
private fun CategoryManageRow(
    ...
    val countFlow = remember(category.id, category.type) { viewModel.countItemsInCategory(category) }
